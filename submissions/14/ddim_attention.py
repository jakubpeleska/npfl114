...
def SelfAttention(inputs, heads):
    ...
    # Pass the `inputs` through a batch normalization.
    hidden = tf.keras.layers.BatchNormalization()(inputs)

    # Considering a single image a 2D collection of feature vectors, reshape `hidden`
    # so that every image is just a linear sequence of feature vectors.
    hidden = tf.keras.layers.Reshape([-1, inputs.shape[3]])(hidden)

    # Pass `hidden` through a multi-head attention. Use the `tf.keras.layers.MultiHeadAttention`
    # with a key dimensionality of `inputs.shape[3] // heads` and with `use_bias=False`.
    hidden = tf.keras.layers.MultiHeadAttention(heads, key_dim=inputs.shape[3] // heads, use_bias=False)(hidden, hidden)

    # Reshape the result so that each image is again a 2D collection of feature vectors.
    hidden = tf.keras.layers.Reshape(inputs.shape[1:])(hidden)
...
class DDIM(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, data: tf.data.Dataset) -> None:
        ...
        for i in range(args.stages):
            ...
            # For the last `args.attention_stages` stages, pass `hidden` through
            # a `SelfAttention`.
        ...
        # Pass `hidden` through a `SelfAttention` block.
        hidden = SelfAttention(hidden, args.attention_heads)
        ...
        for i in reversed(range(args.stages)):
            ...
            # For the first `args.attention_stages` stages (the ones with the smallest
