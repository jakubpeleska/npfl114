...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        }[args.rnn](args.rnn_dim, return_sequences=True)
        
        x = rnn_layer(sequences)
        if args.hidden_layer is not None and args.hidden_layer > 0:
            x = tf.keras.layers.Dense(args.hidden_layer, activation=tf.nn.relu)(x)
