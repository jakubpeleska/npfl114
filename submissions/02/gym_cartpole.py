...
def main(args: argparse.Namespace) -> Optional[tf.keras.Model]:
    ...
    if not args.evaluate:
        ...
        _, input_size = observations.shape
        output_size = 2
        
        # Create the model two-class classification
        # with 1 hidden layer and 2 outputs and softmax activation.
        ...
        tb_callback = tf.keras.callbacks.TensorBoard(
            args.logdir, histogram_freq=1)

        model.fit(
            observations, labels, batch_size=args.batch_size, epochs=args.epochs, callbacks=[
                tb_callback]
        )
        ...
        if args.recodex:
            ...
            score = evaluate_model(model, seed=args.seed,
