...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        self._source_rnn = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(args.rnn_dim, return_sequences=False), merge_mode='sum')
    ...
    def encoder(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        # Run the `self._source_rnn` on the embedded sequences, and return the result.
        return self._source_rnn(embedded)
    ...
    def decoder_training(self, encoded: tf.Tensor, targets: tf.Tensor) -> tf.Tensor:
        ...
        x = self._target_rnn(x, initial_state=[encoded])
